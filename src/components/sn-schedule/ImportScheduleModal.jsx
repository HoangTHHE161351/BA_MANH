...
import { Box, Typography, Button, Grid, Table, TableContainer, TableHead, TableBody, TableRow, TableCell, Paper } from '@mui/material';
...
const ScheduleModalImport = ({ isOpen, onClose }) => {
    ...
    const handleFileUpload = (e) => {
        ...
        reader.onload = (e) => {
            ...
            const filteredDataClass = parsedDataClass.filter(row => row.some(cell => cell !== null && cell !== undefined));
    ...
    const importData = async () => {
        try {
            ...
            const response = await fetch('http://localhost:8080/api/v1/classroom/import-classroom', {
                method: 'POST',
                headers: {
                    ...
                    <ModalContainer>
                        <Box>
                            ...
                            {jsonData.length > 0 && (
                                ...
                                <TableContainer component={Paper} sx={{ mt: 3, maxHeight: 'calc(80vh - 240px)' }}>
                                    <Table stickyHeader>
                                        <TableHead>
                                            <TableRow>
                                                {jsonData[0].map((header, index) => (
                                                    <TableCell sx={{ fontWeight: 'bold', justifyContent: "center" }} key={index}>{header}</TableCell>
                                                ))}
                                            </TableRow>
                                        </TableHead>
                                        <TableBody>
                                            {jsonData.slice(1).map((row, rowIndex) => (
                                                <TableRow key={rowIndex}>
                                                    {row.map((cell, cellIndex) => (
                                                        <TableCell sx={{ justifyContent: "center" }} key={cellIndex}>{cell}</TableCell>
                                                    ))}
                                                </TableRow>
                                            ))}
                                        </TableBody>
